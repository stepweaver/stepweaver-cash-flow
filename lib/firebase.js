...
export const addPersonalBill = async (bill) => {
  try {
    // Extract month and year from dueDate if available
    let month = null;
    let year = null;

    // Only extract month/year if dueDate is a valid date (not empty)
    if (bill.dueDate && bill.dueDate.trim() !== '') {
      const date = new Date(bill.dueDate);
      // Check if the date is valid before extracting month/year
    ...
    return { id: docRef.id, ...bill, month, year };
  } catch (error) {
...
export const getPersonalData = async (year, month) => {
  try {
    ...
    // Get bills for the month using month and year fields
    const billsQuery = query(
      collection(db, 'personalBills'),
      where('month', '==', month),
      where('year', '==', year)
    );
    console.log('Bills query:', { month, year, query: billsQuery });
    ...
    console.log('Bills query results:', { month, year, billsCount: bills.length, bills });
...
export const updatePersonalBill = async (id, updates) => {
  try {
    // If dueDate is being updated, also update month and year
    // Only update month/year if dueDate is a valid date (not empty)
    if (updates.dueDate && updates.dueDate.trim() !== '') {
      const date = new Date(updates.dueDate);
      // Check if the date is valid before extracting month/year
      if (!isNaN(date.getTime())) {
        updates.month = date.getMonth() + 1; // Convert 0-indexed to 1-indexed
        updates.year = date.getFullYear();
      }
      // If dueDate is empty or invalid, don't update month/year - keep existing values
    }
...
export const generateBillsForMonth = async (year, month, templates) => {
  try {
    ...
    console.log('Generated yearMonth:', yearMonth);
