...
export const getPersonalData = async (year, month) => {
  try {
    console.log('getPersonalData called with:', { year, month });

    const monthStr = String(month).padStart(2, '0');
    const yearMonth = `${year}-${monthStr}`;

    console.log('Generated yearMonth:', yearMonth, 'monthStr:', monthStr);
    ...
    console.log('Income query results:', { yearMonth, incomeCount: income.length });
    ...
    console.log('Bills query (dueDate):', { yearMonth, query: billsQuery });
    ...
    console.log('DueDate bills found:', bills.length, 'bills:', bills);
    ...
    console.log('Generated bills query (month/year):', { month, year, query: generatedBillsQuery });
    ...
    console.log('Month/Year bills found:', generatedBills.length, 'bills:', generatedBills);
    ...
    console.log('Bills without dueDate:', billsWithoutDueDate.length, 'bills:', billsWithoutDueDate);
    ...
    console.log('Bills query results:', {
      yearMonth,
      dueDateBillsCount: bills.length,
      generatedBillsCount: billsWithoutDueDate.length,
...
export const generateBillsForMonth = async (year, month, templates) => {
  try {
    console.log('generateBillsForMonth called with:', { year, month, templatesCount: templates.length });

    const monthStr = String(month).padStart(2, '0');
    const yearMonth = `${year}-${monthStr}`;

    console.log('Generated yearMonth:', yearMonth, 'monthStr:', monthStr, 'month param:', month, 'year param:', year);
    ...
    console.log('Existing bills found:', existingBills.length, 'query params:', { month, year });
    ...
    for (const template of templates) {
      ...
      if (existingBill) {
        ...
        console.log('Updating existing bill with:', updatedBill);

        await updateDoc(doc(db, 'personalBills', existingBill.id), updatedBill);
        newBills.push({ id: existingBill.id, ...updatedBill });
        console.log(`Updated existing bill: ${template.name} with blank values`);
      } else {
        ...
        console.log('Creating new bill with:', newBill);

        const docRef = await addDoc(collection(db, 'personalBills'), newBill);
        newBills.push({ id: docRef.id, ...newBill });
        console.log(`Created new bill: ${template.name} with blank values`);
      }
    }

    console.log('generateBillsForMonth completed. Generated/updated bills:', newBills.length);
    return newBills;
  } catch (error) {
