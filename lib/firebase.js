...
export const deletePersonalBillFromCurrentAndFuture = async (billName, currentYear, currentMonth) => {
  try {
    ...
    // First, get all bills with this name (no date filter)
    const billsQuery = query(
      collection(db, 'personalBills'),
      where('name', '==', billName)
    );
    ...
    // Filter bills by date in memory to avoid composite index requirement
    const billsToDelete = billsSnapshot.docs.filter(doc => {
      const billData = doc.data();
      return billData.dueDate >= currentDateStr;
    });

    const deletePromises = billsToDelete.map(doc => deleteDoc(doc.ref));
