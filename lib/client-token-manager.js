...
export const useTokenManager = () => {
  ...
  const getScopedToken = useCallback(async (scope, resourceId = null, additionalClaims = {}) => {
    try {
      ...
      // Request a new scoped token from the server with retry logic
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          const retryResponse = await fetch('/api/token', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (!retryResponse.ok) {
            ...
            throw new Error(retryErrorData.error || 'Failed to get scoped token after retry');
          }

          const retryTokenData = await retryResponse.json();

          // Cache the token with expiry
          tokenCacheRef.current.set(cacheKey, retryTokenData.token);
          tokenExpiryRef.current.set(cacheKey, Date.now() + (retryTokenData.expiresIn * 1000));
  ...
  const createPersonalIncome = useCallback(async (incomeData) => {
    return makeAuthenticatedRequest('/api/personal-data', {
      method: 'POST',
      body: JSON.stringify({ type: 'income', data: incomeData })
    });
  }, [makeAuthenticatedRequest]);
  ...
  const createPersonalBill = useCallback(async (billData) => {
    return makeAuthenticatedRequest('/api/personal-data', {
      method: 'POST',
      body: JSON.stringify({ type: 'bill', data: billData })
    });
  }, [makeAuthenticatedRequest]);
