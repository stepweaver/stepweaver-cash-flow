...
if (getApps().length === 0) {
  try {
    ...
    if (hasMinimalCredentials) {
      ...
      console.warn('⚠️  Insufficient Firebase Admin credentials for initialization');
  ...
  try {
    ...
    console.warn('⚠️  Failed to get Firebase Admin services from existing app:', error.message);
  }
}
